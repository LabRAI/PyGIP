...
class FingerprintGraph(nn.Module):
    ...
    def edge_index(self, thresh=0.5, update=False):
        ...
        if update and self.A_logits.grad is not None:
            grad = self.A_logits.grad; mask = torch.sign(grad)
            new_A = A.clone()
            new_A = torch.where((mask > 0) & (A < 0.5), torch.ones_like(new_A), new_A)
            new_A = torch.where((mask < 0) & (A >= 0.5), torch.zeros_like(new_A), new_A)
            new_A.fill_diagonal_(0.0); new_A = torch.maximum(new_A, new_A.T)
            idx = new_A.nonzero(as_tuple=False)
            if idx.numel() == 0: return torch.empty(2, 0, dtype=torch.long, device=self.device)
            return idx.t().contiguous()
        else:
            A = (A > thresh).float(); A.fill_diagonal_(0.0); A = torch.maximum(A, A.T)
    ...
    def model_response(self, model, require_grad=False):
        ...
        with ctx:
            for fp in self.fps:
                ...
                outs.append(probs)
        return torch.cat(outs, dim=1).flatten()  # [P*C]
...
class GNNFingersDefense(BaseDefense):
    ...
    def __init__(self, dataset, attack_node_fraction: float=0.1,
        ...
        self.P = P; self.fp_nodes = fp_nodes; self.backbone_out = backbone_out
        self.joint_iters = joint_iters; self.verif_lr = verif_lr; self.fp_lr = fp_lr
    ...
    def defend(self, attack_results=None):
        if attack_results is None:
            ...
            attack = GNNFingersAttack(self.dataset, self.attack_node_fraction, device=self.device,
                                      victim_hidden=128, victim_out=self.backbone_out)
    ...
    def _joint_train(self, target_model, pos_models, neg_models):
        ...
        for it in range(1, self.joint_iters+1):
            ...
            batch = torch.stack([X_pos, X_neg], dim=0)  # [2, D]
            logits = self.verifier(batch)
            ...
            batch = torch.stack([X_pos, X_neg], dim=0)
            logits = self.verifier(batch)
            labels = torch.tensor([1, 0], device=self.device, dtype=torch.long)
    ...
    def verify_ownership(self, suspect_model):
        ...
        logits = self.verifier(x.reshape(1, -1))
...
def evaluate(defense, pos_models, neg_models):
    ...
    aruc = (tpr + tnr) / 2
    # mean test accuracy of target model
    target_model = defense.target_model
    target_model.eval()
    correct, total = 0, 0
    with torch.no_grad():
        for batch in defense.dataset.test_data:
            batch = batch.to(defense.device)
            logits, _ = target_model(batch)
            pred = logits.argmax(dim=1)
            correct += (pred == batch.y).sum().item()
            total += batch.y.size(0)
    mean_test_accuracy = correct / max(1, total)
