...
- Uses same dataset/model structure and device conventions as the main framework
"""
...
# ------------------------------------------------
# PyGIP integration (dataset + models)
# ------------------------------------------------
try:
    ...
    raise ImportError("Please ensure pygip is in PYTHONPATH before running this script.") from e
...
def train_model(model, data, train_mask, epochs=50, lr=0.01, device=None):
    """Train a simple model for fixed epochs."""
...
def get_posteriors(model, data, nodes, device=None):
    """Get softmax posteriors for given node indices."""
    device = device or get_device()
    model = model.to(device)
    data = data.to(device)
    model.eval()
...
def build_model(model_type, in_dim, out_dim, layers=2):
    """Return the desired model backbone."""
    if model_type == "GCN":
...
# Threshold tuning
# ------------------------------------------------
def tune_threshold(Fs_star, Fs, Find, data, query_nodes, device=None):
    """Find similarity threshold maximizing accuracy."""
    device = device or get_device()
    scores, labels = [], []

    for star in Fs_star:
        probs_star = get_posteriors(star, data, query_nodes, device=device)
        # genuine (1)
        for surrogate in Fs:
            sim = cosine_similarity(probs_star, get_posteriors(surrogate, data, query_nodes, device=device)).mean()
            scores.append(sim)
            labels.append(1)
        # impostors (0)
        for ind in Find:
            sim = cosine_similarity(probs_star, get_posteriors(ind, data, query_nodes, device=device)).mean()
            scores.append(sim)
            labels.append(0)
...
# ------------------------------------------------
# Single run (Table 4 cell)
# ------------------------------------------------
def run_bgrove_experiment(dataset_cls, condition="CondA ✓", setting="I", device=None):
    device = device or get_device()
    ...
    query_nodes = torch.randperm(data.num_nodes, device=device)[:num_queries]
    thr = tune_threshold(Fs_star, Fs, Find, data, query_nodes, device=device)

    true_labels, pred_labels = [], []
    for model in Fs + Find:
        for star in Fs_star:
            sim = cosine_similarity(
                get_posteriors(model, data, query_nodes, device=device),
                get_posteriors(star, data, query_nodes, device=device)
            ).mean()
...
# ------------------------------------------------
def run_multi(dataset_cls, condition, setting, device=None, seeds=(0, 1, 2, 3, 4)):
    device = device or get_device()
...
if __name__ == "__main__":
    ...
    for DatasetClass in datasets:
        for cond in conditions:
            for setting in settings:
                ...
                FPR, FNR, ACC = run_multi(DatasetClass, cond, setting, device)
    ...
    out_path = "results/BGrOVe_table4.csv"
    df.to_csv(out_path)
